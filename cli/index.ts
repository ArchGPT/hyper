...
args.command(
  ...
  (name, args_) => {
    ...
    commandPromise = api
      ...
      .then(() =>
        console.log(chalk.green(`${pluginName} uninstalled successfully!`)),
      )
      .catch((err) => console.error(chalk.red(err)));
  },
  ["u", "rm", "remove"],
    ...
    commandPromise = lsRemote(query)
      .then((entries) => {
        if (entries.length === 0) {
          spinner.fail();
          console.error(
            chalk.red(`Your search '${query}' did not match any plugins`),
          );
          console.error(
            `${chalk.red("Try")} ${chalk.green("hyper ls-remote")}`,
          );
          process.exit(1);
        } else {
...
const main = (argv: string[]) => {
  ...
  const args_ = args.sub.map((arg) => {
    ...
    if (!existsSync(cwd)) {
      console.error(
        chalk.red(`Error! Directory or file does not exist: ${cwd}`),
      );
      process.exit(1);
    }
    return cwd;
